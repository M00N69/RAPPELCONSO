...
try:
    ...
    st.error(
        "Clé API Gemini Pro manquante. Veuillez configurer la clé 'api_key' dans les secrets Streamlit."
    )
    genai = None
...
def load_data(url, start_date=START_DATE):
    """Loads and preprocesses the recall data from API, once with a larger page size."""
    all_records = []
    ...
    # Construct base URL with date filter and larger page size for initial load
    base_url_with_date_filter = f"{url}&refine.date_de_publication>={start_date_str}&refine.date_de_publication<={today_str}&rows={API_PAGE_SIZE}"  # Larger page size

    with st.spinner("Chargement initial des données (depuis 2022)..."):  # Initial loading spinner
        request_url = base_url_with_date_filter
        print(f"Requesting URL for initial load: {request_url}")  # Debug: Print URL
        try:
            ...
            if not records:  # Break if no records are returned
                print("No records from API for initial load.")  # Debug
            else:
                all_records.extend([rec["fields"] for rec in records])
                fetched_count = len(
                    records
                )  # For initial load, fetched_count is just the number of records

                print(
                    f"Fetched {len(records)} records in initial load. Total fetched: {fetched_count}"
                )  # Debug: Record counts

                if "total_count" in data:  # Use total_count from API if available
                    total_count = min(
                        data["total_count"], MAX_RECORDS_LIMIT
                    )  # Apply MAX_RECORDS_LIMIT as a cap
                elif not total_count_warning_displayed:  # Display warning only once
                    st.warning(
                        "Clé 'total_count' manquante dans la réponse de l'API. Utilisation d'une limite maximale de rappels."
                    )
                    total_count_warning_displayed = True  # Set flag to True

        except requests.exceptions.RequestException as e:
            st.error(f"Erreur de requête API lors du chargement initial: {e}")
            print(f"API Request Error during initial load: {e}")  # Debug: Print error details
            return pd.DataFrame()  # Return empty DataFrame in case of error
        except KeyError as e:  # Catch potential KeyError for 'total_count' or 'records'
            st.error(f"Erreur de structure JSON de l'API lors du chargement initial: clé manquante {e}")
            print(f"JSON Structure Error during initial load: Missing key {e}")  # Debug: JSON error
            return pd.DataFrame()
        except requests.exceptions.Timeout:
            st.error(
                f"Délai d'attente dépassé lors de la requête initiale à l'API. L'API RappelConso est peut-être lente ou inaccessible. Veuillez réessayer plus tard."
            )
            print("API Timeout Error during initial load")  # Debug: Timeout
            return pd.DataFrame()

    if not all_records:
        print("No records loaded in total after initial load.")  # Debug: No data at all
        return pd.DataFrame()  # Return empty DataFrame if no records fetched

    df = pd.DataFrame(all_records)
    print(f"DataFrame created with {len(df)} rows after initial load.")  # Debug: DataFrame size
    ...
    df = df.dropna(
        subset=["date_de_publication"]
    )  # Clean invalid dates if any remain after API date filter (should not be needed much)
...
def filter_data(df, subcategories, risks, search_term, date_range):  # Added date_range parameter
    ...
    if subcategories:
        filtered_df = filtered_df[
            filtered_df["sous_categorie_de_produit"].isin(subcategories)
        ]
    if risks:
        filtered_df = filtered_df[
            filtered_df["risques_encourus_par_le_consommateur"].isin(risks)
        ]
    ...
    if not data.empty:
        ...
        for idx, row in current_recalls.iterrows():
            ...
            <div class="recall-container">
                <img src="{row.get('liens_vers_les_images', '')}" class="recall-image" alt="Image du produit: {row.get('noms_des_modeles_ou_references', 'N/A')}">
                <div class="recall-content">
                    <div class="recall-title">{row.get('noms_des_modeles_ou_references', 'N/A')}</div>
                    <div class="recall-date">{row['date_de_publication'].strftime('%d/%m/%Y') if isinstance(row.get('date_de_publication'), date) else 'N/A'}</div>
                    <div class="recall-description">
                        <strong>Marque:</strong> {row.get('nom_de_la_marque_du_produit', 'N/A')}<br>
                        <strong>Motif du rappel:</strong> {row.get('motif_du_rappel', 'N/A')}
                    </div>
...
def display_visualizations(data):
    ...
    if not data.empty:
        ...
        if not filtered_categories_data.empty and not filtered_legal_data.empty:
            ...
            st.warning(
                "Données insuffisantes pour afficher certains graphiques après filtrage."
            )  # Changed to warning
    else:
        st.error(
            "Aucune donnée disponible pour les visualisations avec les filtres actuels."
        )
...
def display_top_charts(data):
    ...
    with col1:
        top_subcategories_df = (
            data["sous_categorie_de_produit"]
            .value_counts()
            .head(5)
            .reset_index()
        )  # Create DataFrame
        top_subcategories_df.columns = [
            "Sous_categorie",
            "Nombre_de_rappels",
        ...
        if not top_subcategories_df.empty:
            fig_top_subcategories = px.bar(
                top_subcategories_df,
                x="Sous_categorie",  # Column name
                y="Nombre_de_rappels",  # Column name
                labels={
                    "Sous_categorie": "Sous-catégories",
                    "Nombre_de_rappels": "Nombre de rappels",
                },
            ...
            st.info(
                "Pas assez de données de sous-catégories pour afficher le graphique Top 5."
            )

    with col2:
        top_risks_df = (
            data["risques_encourus_par_le_consommateur"]
            .value_counts()
            .head(5)
            .reset_index()
        )  # Create DataFrame
        top_risks_df.columns = [
            "Risque",
            "Nombre_de_rappels",
        ]  # Rename columns for clarity
...
def get_relevant_data_as_text(user_question, data):
    ...
    selected_rows = data[
        data.apply(
            lambda row: any(
                keyword in str(val).lower() for keyword in keywords for val in row
            ),
            axis=1,
        )
    ...
    for index, row in selected_rows.iterrows():
        context += (
            f"- Date de Publication: {row['date_de_publication'].strftime('%d/%m/%Y') if isinstance(row.get('date_de_publication'), date) else 'N/A'}\n"
        )
        context += (
            f"- Nom du Produit: {row.get('noms_des_modeles_ou_references', 'N/A')}\n"
        )
...
def main():
    ...
    if df.empty:  # Stop if initial data load fails - HANDLE EMPTY DATAFRAME HERE
        st.error(
            "Impossible de charger les données de rappels depuis l'API. Veuillez vérifier la console pour plus de détails. L'API RappelConso est peut-être inaccessible ou ne retourne pas de données pour la période spécifiée."
        )
    ...
    page = st.sidebar.selectbox(
        "Choisir Page", ["Page principale", "Visualisation", "Details", "Chatbot"]
    )

    with st.sidebar.expander("Filtres avancés", expanded=False):
        # Sub-category and risks filters (none selected by default)
        selected_subcategories = st.multiselect(
            "Sous-catégories", options=all_subcategories, default=[]
        )
        selected_risks = st.multiselect(
            "Risques", options=all_risks, default=[]
        )
    ...
    filtered_data = filter_data(
        df, selected_subcategories, selected_risks, search_term, selected_dates
    )  # Pass selected_dates to filter_data
    ...
    if page == "Page principale":
        ...
        st.write(
            "Explorez les tendances et répartitions des rappels de produits alimentaires à travers ces graphiques interactifs."
        )
        ...
        st.write(
            "Consultez et téléchargez un tableau détaillé des rappels de produits, filtré selon vos préférences."
        )
        ...
        if not filtered_data.empty:
            ...
            st.info(
                "Aucune donnée à afficher avec les filtres sélectionnés. Ajustez vos filtres pour voir les détails."
            )  # Changed to info
            ...
            if user_input.strip() == "":
                ...
                with st.spinner("Réflexion du Chatbot..."):
                    try:
                        ...
                        relevant_data = get_relevant_data_as_text(
                            user_input, filtered_data
                        )
